...
//        String[] array = splitMyWords(s,"(?i)^[a-zA-Z\\s]+$((?<=[\\w\\p{Ll}])\\s*,\\s*(?=[\\w\\p{Ll}]))|((?<=[\\w\\p{Ll}])\\s*,\\s*(?=[\\w\\p{Lu}]))|((?<=[\\w\\p{Lu}])\\s*,\\s*(?=[\\w\\p{Ll}]))|((?<=[\\w\\p{Lu}])\\s*,\\s*(?=[\\w\\p{Lu}]))|((?<=[\\w\\p{Lu}])*,\\s*(?=[\\w\\p{Lu}]))|((?<=[\\w\\p{Ll}])\\s*,\\s*(?=«[\\w\\p{Lu}]))|((?<=»),\\s*(?=«[\\w\\p{Lu}]))|((?<=[\\w\\p{Lu}]\\))\\s*,\\s*(?=[\\w\\p{Ll}]))|((?<=[\\w\\p{Ll}])\\s*,\\s*(?=[￼\\w\\p{Lu}]))");
//        ArrayList<String> words = new ArrayList<>();
//        for (String word : array) {
//            words.add(word.trim());
//        }
...
//    }
    ...
    //    public static ArrayList<String> getListOfCommaSeparatedWords(String s) {
//        String[] array = splitMyWords(s,"((?<=[\\w\\p{Ll}])\\s*,\\s*(?=[\\w\\p{Ll}]))|((?<=[\\w\\p{Ll}])\\s*,\\s*(?=[\\w\\p{Lu}]))|((?<=[\\w\\p{Lu}])\\s*,\\s*(?=[\\w\\p{Ll}]))|((?<=[\\w\\p{Lu}])\\s*,\\s*(?=[\\w\\p{Lu}]))|((?<=[\\w\\p{Lu}])*,\\s*(?=[\\w\\p{Lu}]))|((?<=[\\w\\p{Ll}])\\s*,\\s*(?=«[\\w\\p{Lu}]))|((?<=»),\\s*(?=«[\\w\\p{Lu}]))|((?<=[\\w\\p{Lu}]\\))\\s*,\\s*(?=[\\w\\p{Ll}]))|((?<=[\\w\\p{Ll}])\\s*,\\s*(?=[￼\\w\\p{Lu}]))");
//        ArrayList<String> words = new ArrayList<>();
//        for (String word : array) {
//            words.add(word.trim());
//        }
...
//    }
    public static ArrayList<String> getListOfCommaSeparatedWords(String s) {
        String[] array = splitMyWords(s, "((?<=.)\\s*,\\s*(?=.))");
        ArrayList<String> words = new ArrayList<>();
        for (String word : array) {
